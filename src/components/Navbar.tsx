...
export default function Navbar({ onMember, isMember, isAdmin, onAdmin }: Props) {
    ...
    <>
      ...
      >
        <div className="max-w-7xl mx-auto px-6 md:px-10 h-16 md:h-20 flex items-center justify-between gap-6">
          <a href="#" className="flex items-center gap-3 group" aria-label="SACCB — Accueil">
            <span className="relative w-9 h-9 flex items-center justify-center border border-[color:var(--ink)] group-hover:bg-[color:var(--ink)] transition-colors duration-500">
              <Shuttlecock className="w-5 h-5 transition-colors duration-500 text-[color:var(--ink)] group-hover:text-[color:var(--bone)]" color="currentColor" />
            </span>
            <span className="flex flex-col leading-none">
              <span className="font-display text-2xl tracking-[0.15em] text-[color:var(--ink)]">SACCB</span>
              <span className="text-[9px] tracking-[0.3em] uppercase text-[color:var(--muted)] font-semibold mt-0.5" style={{ fontFamily: "Oswald, sans-serif" }}>
                Sainte-Adresse
              </span>
            </span>
          </a>
          ...
          <nav aria-label="Navigation principale" className="hidden lg:flex items-center gap-8">
            {LINKS.map((l) => (
              <a
                key={l.href}
                href={l.href}
                className="group relative flex items-center gap-2 text-[12px] uppercase font-semibold text-[color:var(--ink)] transition-opacity hover:opacity-100 opacity-75"
      ...
      >
        <div className="h-full flex flex-col pt-24 pb-10 px-6 md:px-10 overflow-y-auto">
          <nav aria-label="Navigation mobile" className="flex-1 flex flex-col justify-center">
            <ul className="space-y-3">
              {LINKS.map((l, i) => (
                <li
                  key={l.href}
                  className="border-b border-[color:var(--line)]"
                  style={{ animation: open ? `fadeUp 0.6s ${i * 0.04 + 0.1}s both cubic-bezier(0.22,1,0.36,1)` : "none" }}
                >
                  <a
                    href={l.href}
                  ...
                  >
                    <span className="flex items-baseline gap-4">
                      <span className="text-[10px] tracking-[0.3em] text-[color:var(--muted)] font-semibold" style={{ fontFamily: "Oswald, sans-serif" }}>
                        {l.num}
                      </span>
          ...
          <div className="pt-8 flex flex-col gap-3">
            {isAdmin && onAdmin && (
              <button
                ...
                style={{ fontFamily: "Oswald, sans-serif", letterSpacing: "0.22em" }}
              >
            ...
            <div className="mt-6 flex items-center gap-2 text-[11px] uppercase text-[color:var(--muted)]" style={{ fontFamily: "Oswald, sans-serif", letterSpacing: "0.22em" }}>

...
function TournoiCard({ t, inscrits, memberSession, onLoginRequest }: {
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5 }} className="glass-sport overflow-hidden">
        ...
        <div className="flex flex-wrap items-start justify-between gap-4 mb-4">
          <div>
            ...
            <div className="flex flex-wrap items-center gap-3 mt-2 text-sm text-slate-500">
              ...
              {t.type && <span className="px-3 py-1 rounded-full bg-blue-100 text-blue-700 text-xs font-semibold uppercase">Double {t.type}</span>}
              {t.quota && <span className="flex items-center gap-1.5 text-emerald-600"><Users className="w-4 h-4" /> {inscrits.length} / {t.quota} doubles</span>}
            </div>
